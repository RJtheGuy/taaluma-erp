...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            # Step 2: Create Users FIRST (before warehouses)
            self.stdout.write('\n👥 Step 2: Creating Users...')
            users = self.create_users_without_warehouses(organizations, groups)
            
            # Step 3: Create Warehouses
            self.stdout.write('\n🏭 Step 3: Creating Warehouses...')
            warehouses = self.create_warehouses(organizations, users)
            
            # Step 4: Assign Warehouses to Users
            self.stdout.write('\n🔗 Step 4: Assigning Warehouses to Users...')
            self.assign_warehouses_to_users(users, warehouses)
            
            # Step 5: Create Products
            self.stdout.write(f'\n📦 Step 5: Creating {options["products"]} Products...')
            products = self.create_products(options['products'], users, organizations)
            
            # Step 6: Create Stock Records
            self.stdout.write('\n📊 Step 6: Creating Stock Records...')
            self.create_stock(products, warehouses, users)
            
            # Step 7: Create Customers
            self.stdout.write(f'\n🛒 Step 7: Creating {options["customers"]} Customers...')
            customers = self.create_customers(options['customers'], users, organizations)
            
            # Step 8: Create Orders
            self.stdout.write(f'\n📝 Step 8: Creating {options["orders"]} Orders...')
            self.create_orders(options['orders'], customers, products, warehouses, users)
    ...
    def create_users_without_warehouses(self, organizations, groups):
        """Create users WITHOUT warehouse assignments first"""
        users = {}
        ...
        user_data = [
            ...
            },
            {
                ...
                'is_superuser': False,
                'groups': ['Staff'],
                'warehouse_org': 'african-heritage',
                'warehouse_index': 0
            },
        ...
        for data in user_data:
            ...
            user, created = User.objects.get_or_create(
                ...
                defaults={
                    ...
                    'is_staff': data['is_staff'],
                    'is_superuser': data['is_superuser'],
                    'assigned_warehouse': None  # Will assign later
                }
            )
            ...
            if created:
                ...
                org_name = org.name if org else 'Platform Admin'
                self.stdout.write(f'  ✓ {data["username"]} → {org_name}')
            else:
                self.stdout.write(f'  • Exists: {data["username"]}')
    ...
    def create_warehouses(self, organizations, users):
        """Create warehouses for each organization"""
        warehouses = {}
        ...
        for org_slug, wh_list in warehouse_data.items():
            ...
            # Get admin user for this org to use as created_by
            admin_user = users.get(f'admin.{org_slug.replace("-", "")}') or users.get(f'{org_slug.replace("-", "_")}_admin') or users['superadmin']
    ...
    def assign_warehouses_to_users(self, users, warehouses):
        """Assign warehouses to users after warehouses are created"""
        assignments = {
            'staff.conegliano': ('ethnic-world', 0),
            'staff.african': ('african-heritage', 0),
        }
        
        for username, (org_slug, wh_index) in assignments.items():
            if username in users:
                user = users[username]
                warehouse = warehouses[org_slug][wh_index]
                user.assigned_warehouse = warehouse
                user.save()
                self.stdout.write(f'  ✓ {username} → {warehouse.name}')
    ...
    def create_products(self, count, users, organizations):
        ...
        for org_slug, org in organizations.items():
            ...
            if not admin_user:
                self.stdout.write(f'  ⚠ No admin user found for {org_slug}, skipping...')
                continue
